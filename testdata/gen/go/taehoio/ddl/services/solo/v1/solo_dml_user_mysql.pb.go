...
//go:generate mockgen -package solov1 -self_package "github.com/taehoio/protoc-gen-go-ddl/gen/go/ddl/services/solo/v1" -source ./solo_dml_user_mysql.pb.go -destination ./solo_dml_user_mysql_mock.pb.go -mock_names UserRecorder=MockUserRecorder "github.com/taehoio/protoc-gen-go-ddl/gen/go/ddl/services/solo/v1" UserRecorder

const (
	userInsertStmt = "INSERT INTO `user` " + `(
			id, created_at, updated_at, deleted_at, solo
		) VALUES (
			?, ?, ?, ?, ?
		)
	`
